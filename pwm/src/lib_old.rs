//! This PWM crate was partially generated by qwen. The purpose of this crate is to drive the PWM
//! that is output through the GPIOs for LEDc and the MOSFETs.
...
/// Represents the four mosfets and channels that can connect to a timer to drive the PWM.
...
/// The different kinds of signals that a pin can receive. `UnfreezePin` will set the frozen flag
/// false. I forgot why I don't have a FreezePin action.
...
/// Represents the current output state of a PWM output. Originally with four outputs. Now, we only
/// need two outputs.
...
/// Needed so that other threads can safely access the output state. 
