...
impl SetupMethodState {
    ...
    fn draw_tooltip(&mut self, current_tooltip: u8, io_handles: &mut IOHandles<'_>) -> anyhow::Result<()> {
        let tooltip_top = Text::with_alignment(TOOLTIPS_TOP[current_tooltip as usize][Lang::En], Point::new(64, 80), io_handles.font.clone(), Alignment::Center);
        self.undraw_tooltip[0] = tooltip_top.bounding_box();
        tooltip_top.draw(io_handles.screen.borrow_mut())?;
        let tooltip_bottom = Text::with_alignment(TOOLTIPS_BOTTOM[current_tooltip as usize][Lang::En], Point::new(64, 90), io_handles.font.clone(), Alignment::Center);
...
impl MenuBehaviour for SetupMethodState {
    async fn init(&mut self, spawner: Spawner, io_handles: &mut IOHandles<'_>) -> anyhow::Result<MenuSignal> {
        ...
        RefCell::borrow_mut(&io_handles.font.font).set_size(FontSize::Sz7)?;
        Text::with_alignment(TEXT_SETUP_PB[Lang::En], Point::new(64, 20), io_handles.font.clone(), Alignment::Center).draw(io_handles.screen.borrow_mut())?;
        Text::with_alignment(TEXT_SETUP_PB_A[Lang::En], Point::new(64, 30), io_handles.font.clone(), Alignment::Center).draw(io_handles.screen.borrow_mut())?;
        Text::with_alignment(TEXT_WIFI_SETUP[Lang::En], Point::new(64, 50), io_handles.font.clone(), Alignment::Center).draw(io_handles.screen.borrow_mut())?;
        Text::with_alignment(TEXT_STANDALONE_SETUP[Lang::En], Point::new(64, 60), io_handles.font.clone(), Alignment::Center).draw(io_handles.screen.borrow_mut())?;
            ...
            match result {
                ...
                Either::Second(b) => {
                    match b.button_type {
                        ...
                        ButtonType::Left => return Ok(MenuSignal::Transition(MenuSelection::LanguageMenu)),
                        _ => {}
                    }
                }
            }
