...
pub enum ST7735Error {
    PinError(SourceFn, Pin),
    SPIError(SourceFn)
}

impl core::error::Error for ST7735Error { }
...
impl core::fmt::Display for SourceFn {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            SourceFn::WriteCommand => write!(f, "write_command"),
            SourceFn::StartData => write!(f, "start_data"),
            SourceFn::HardReset => write!(f, "hard_reset"),
            SourceFn::WriteData => write!(f, "write_data"),
        }
    }
}

impl core::fmt::Display for ST7735Error {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ST7735Error::PinError(src, pin) => write!(f, "Pin error in function {}, on pin {}", src, pin),
            ST7735Error::SPIError(src) => write!(f, "SPI Error in function {}!", src),
        }
    }
}

type Result<T> = core::result::Result<T, ST7735Error>;
    ...
    RST: OutputPin,
{
        ...
        DELAY: DelayNs,
    {
        if let Some(rst) = &mut self.rst {
            rst.set_high().map_err(|_| ST7735Error::PinError(SourceFn::HardReset, Pin::Rst))?;
            delay.delay_ms(10);
            rst.set_low().map_err(|_| ST7735Error::PinError(SourceFn::HardReset, Pin::Rst))?;
            delay.delay_ms(10);
            rst.set_high().map_err(|_| ST7735Error::PinError(SourceFn::HardReset, Pin::Rst))?;
        }
        Ok(())
    ...
    fn write_command(&mut self, command: Instruction, params: &[u8]) -> Result<()> {
        self.dc.set_low().map_err(|_| ST7735Error::PinError(SourceFn::WriteCommand, Pin::Dc))?;
        self.spi.write(&[command as u8]).map_err(|_| ST7735Error::SPIError(SourceFn::WriteCommand))?;
        if !params.is_empty() {
            self.start_data()?;
    ...
    fn start_data(&mut self) -> Result<()> {
        self.dc.set_high().map_err(|_| ST7735Error::PinError(SourceFn::StartData, Pin::Dc))
    }

    fn write_data(&mut self, data: &[u8]) -> Result<()> {
        self.spi.write(data).map_err(|_| ST7735Error::SPIError(SourceFn::WriteData))
    }
