...
static esp_err_t favicon_get_handler(httpd_req_t *req)
{
    ...
    char k = 0;
    httpd_resp_send(req, (const char *)&k, 1);
    return ESP_OK;
}
...
static esp_err_t http_resp_dir_html(httpd_req_t *req, const char *dirpath)
{
    ...
    unsigned char* upload_script_start = malloc(4096 * sizeof(char));
    size_t newLen = 0;
    FILE* uscript = fopen("/mainfs/upload_script.html", "r");
    if(uscript) {
        newLen = fread(upload_script_start, sizeof(char), 4095, uscript);
        if(ferror(uscript) != 0) {
            ets_printf("ERR IN FILE READING UPLOAD SCRIPT\n");
        } else {
            upload_script_start[newLen+1] = '\0';
        }
        fclose(uscript);
    }
    size_t upload_script_size = newLen;
...
static esp_err_t get_language_handler(httpd_req_t *req) {
    const char get_handler_resp[][2] = {"0", "1", "2", "3", "4", "5", "6", "7", "8"};
