...
static uint8_t _process(rotary_encoder_info_t* info) {
	uint8_t event = 0;
	if (info != NULL) {
		uint8_t pin_state = (gpio_get_level(info->pin_b) << 1) | gpio_get_level(info->pin_a); // Get state of input pins.

		#ifdef ROTARY_ENCODER_DEBUG
			uint8_t old_state = info->table_state;
		#endif
		info->table_state = info->table[info->table_state & 0xf][pin_state]; // Determine new state from the pins and state table.

		event = info->table_state & 0x30; // Return emit bits, i.e. the generated event.
		#ifdef ROTARY_ENCODER_DEBUG
