...
- Unless specified, "Large" font is 24/28px, "Medium" font is 18/24px, "Standard" font is 14/18px, and "Small" text is 12/14px. "H" font is 42/42px (used only for big number in home screen). All sizes are shown in non-CJK/CJK sizes, respectively. Ideally, CJK and non-CJK text will not coexist in the same text field, but if it does, default to the size of the first character.
